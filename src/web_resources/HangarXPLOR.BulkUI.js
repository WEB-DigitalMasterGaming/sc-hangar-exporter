...
HangarXPLOR.BulkUI = function()
{
  ...
  // One file is the product: hangar.json carries ships AND paints, and the
  // app tells the rows apart on its own. CSV and ships-only JSON stay for
  // the other tools that ingest them; a paints-only button is a subset
  // nobody needs to choose.
  HangarXPLOR.$bulkUI.$downloads.append(HangarXPLOR.Button('Download Hangar', 'download js-download-hangar', HangarXPLOR._callbacks.DownloadHangarJSON));
